...
void GameBgLayer::initClound2(){
    Size size = Director::getInstance()->getWinSize();
    Sprite* m_Cloud1 = Sprite::createWithSpriteFrameName("game_cloud1.png");
    m_Cloud1->setAnchorPoint(Vec2(0, 0));
    m_Cloud1->setTag(101);
    m_Cloud1->setPosition(size.width, size.height/2);
    m_bg2->addChild(m_Cloud1);
    
    MoveTo* mvTo1 = MoveTo::create(48, Point(-m_Cloud1->getContentSize().width,m_Cloud1->getPositionY()));
    Sequence* seq1 = Sequence::create(mvTo1,CCCallFunc::create( CC_CALLBACK_0(GameBgLayer::initClound21Position, this)),NULL);
    m_Cloud1->runAction(RepeatForever::create(seq1));
    ...
    m_Cloud2->setTag(102);
    m_Cloud2->setPosition(size.width+size.width/4, size.height*3/4);
    m_bg2->addChild(m_Cloud2);
    MoveTo* mvTo2 = MoveTo::create(60, Point(-m_Cloud2->getContentSize().width,m_Cloud2->getPositionY()));
    Sequence* seq2 = Sequence::create(mvTo2,CCCallFunc::create( CC_CALLBACK_0(GameBgLayer::initClound22Position, this)),NULL);
    m_Cloud2->runAction(RepeatForever::create(seq2));
    ...
    m_Cloud3->setAnchorPoint(Vec2(0,0));
    m_Cloud3->setTag(103);
    m_Cloud3->setPosition(size.width+size.width/10, size.height*5/8);
    m_bg2->addChild(m_Cloud3);
    MoveTo* mvTo3 = MoveTo::create(72, Point(-m_Cloud3->getContentSize().width,m_Cloud3->getPositionY()));
    Sequence* seq3 = Sequence::create(mvTo3,CCCallFunc::create( CC_CALLBACK_0(GameBgLayer::initClound23Position, this)),NULL);
    m_Cloud3->runAction(RepeatForever::create(seq3));
    
}
void GameBgLayer::initClound11Position(){
    Size size = Director::getInstance()->getWinSize();
    Node* sp = m_bg1->getChildByTag(101);
    sp->setPosition(size.width, size.height/2);
}
void GameBgLayer::initClound12Position(){
    Size size = Director::getInstance()->getWinSize();
    Node* sp = m_bg1->getChildByTag(102);
    sp->setPosition(size.width+size.width/4, size.height/2);
}
void GameBgLayer::initClound13Position(){
    Size size = Director::getInstance()->getWinSize();
    Node* sp = m_bg1->getChildByTag(103);
    sp->setPosition(size.width+size.width/2, size.height/2);
}
void GameBgLayer::initClound21Position(){
    Size size = Director::getInstance()->getWinSize();
    Node* sp = m_bg2->getChildByTag(101);
    sp->setPosition(size.width, size.height/2);
}
void GameBgLayer::initClound22Position(){
    Size size = Director::getInstance()->getWinSize();
    Node* sp = m_bg2->getChildByTag(102);
    sp->setPosition(size.width+size.width/4, size.height/2);
}
void GameBgLayer::initClound23Position(){
    Size size = Director::getInstance()->getWinSize();
    Node* sp = m_bg2->getChildByTag(103);
    sp->setPosition(size.width+size.width/2, size.height/2);
}
